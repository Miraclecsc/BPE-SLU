...
def find_error(choice):
    assert choice in ['train', 'dev']
    model.eval()
    dataset = train_dataset if choice == 'train' else dev_dataset
    incorrect_predictions = []  # 新增列表来存储预测错误的样本
    with torch.no_grad():
        for i in range(0, len(dataset), args.batch_size):
            cur_dataset = dataset[i: i + args.batch_size]
            current_batch = from_example_list(
            ...
            for j in range(len(current_batch)):
                if any([l.split('-')[-1] not in current_batch.utt[j] for l in pred[j]]):
                    print(current_batch.utt[j], pred[j], label[j])
                if set(pred[j]) != set(label[j]):  # 检查预测是否正确
                    incorrect_predictions.append(
                        (current_batch.utt[j], label[j]))  # 存储错误预测的样本
    torch.cuda.empty_cache()
    gc.collect()
    return incorrect_predictions  # 返回错误预测的样本列表
...
if not args.testing:
    ...
    metrics, dev_loss = decode('dev')
    # predict()
    # error_dataset = find_error('train')
    # save to json file
    # with open('error_dataset.json', 'w') as f:
    #     json.dump(error_dataset, f, ensure_ascii=False)

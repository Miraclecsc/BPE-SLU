...
class Example():
    ...
    def configuration(cls, root, train_path=None, word2vec_path=None, tokenizer_name=None, extend_cais=False, extend_ecdt=False):
        ...
        cls.word_vocab = Vocab(padding=True, unk=True,
                               filepath=train_path)  # no use now
        cls.word2vec = Word2vecUtils(word2vec_path)  # 词向量
        cls.label_vocab = LabelVocab(
            root, extend_cais=extend_cais, extend_ecdt=extend_ecdt)  # ['B', 'I', 'O', '<pad>']
        cls.tokenizer = BertTokenizer.from_pretrained(tokenizer_name)

    @classmethod
    def load_dataset(cls, data_path, extra_data1=None, extra_data2=None, testing=False):
        datas = json.load(open(data_path, 'r', encoding='utf-8'))

        examples = [cls(utt, testing) for data in datas for utt in data]

        if extra_data1 is not None:
            datas = json.load(open(extra_data1, 'r', encoding='utf-8'))
            extra_examples_1 = [cls(utt, testing)
                                for data in datas for utt in data]
            examples += extra_examples_1

        if extra_data2 is not None:
            datas = json.load(open(extra_data2, 'r', encoding='utf-8'))
            extra_examples_2 = [cls(utt, testing)
                                for data in datas for utt in data]
            examples += extra_examples_2

        return examples

    def __init__(self, ex: dict, did, testing=False):
        ...
        for slot in self.slot:
            ...
            if bidx != -1:
                ...
                self.acts[bidx: bidx +
                          len(value)] = [slot.split('-')[0]] * len(value)

                self.slots[bidx: bidx +
                           len(value)] = [slot.split('-')[1]] * len(value)

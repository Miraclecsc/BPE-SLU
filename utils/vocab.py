...
class LabelVocab():
    ...
    def from_filepath(self, root):
        ...
        ontologies_extend = []
        if self.extend_cais:
            ontologies_extend.append(json.load(open(os.path.join(root, 'ontology_cais.json'), 'r', encoding='utf-8')))
        if self.extend_ecdt:
            ontologies_extend.append(json.load(open(os.path.join(root, 'ontology_ecdt.json'), 'r', encoding='utf-8')))

...
def from_example_list(args, ex_list, device='cpu', train=True):
    ...
    input_ids = [ex.input_idx + [pad_idx] *
                 (max_len - len(ex.input_idx)) for ex in ex_list]
    ...
    batch.tag_mask = torch.tensor(
        [[1] * max_len for _ in range(len(ex_list))], device=device)
    ...
    if train:
        ...
        tag_ids = [ex.tag_id + [tag_pad_idx] *
                   (max_tag_lens - len(ex.tag_id)) for ex in ex_list]
        act_ids = [ex.act_id + [tag_pad_idx] *
                   (max_tag_lens - len(ex.act_id)) for ex in ex_list]
        slot_ids = [ex.slot_id + [tag_pad_idx] *
                    (max_tag_lens - len(ex.slot_id)) for ex in ex_list]
        sep_tag_ids = [ex.sep_tag_id + [tag_pad_idx] *
                       (max_tag_lens - len(ex.sep_tag_id)) for ex in ex_list]

        tag_mask = [[1] * len(ex.tag_id) + [0] *
                    (max_tag_lens - len(ex.tag_id)) for ex in ex_list]
        ...
        batch.slot_ids = torch.tensor(
            slot_ids, dtype=torch.long, device=device)
        batch.sep_tag_ids = torch.tensor(
            sep_tag_ids, dtype=torch.long, device=device)
        batch.tag_mask = torch.tensor(
            tag_mask, dtype=torch.float, device=device)
